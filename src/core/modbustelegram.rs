...
impl ModbusTelegram {
    ...
    pub fn new_from_bytes(bytes: &Vec<u8>) -> Option<ModbusTelegram> {
        ...
        if bytes.len() > 9 {
            let response_transaction_identifier: Option<u16> =
                extract_word_from_bytearray(&bytes, 0);
            ...
            let response_payload: Option<Vec<u8>> =
                extract_payload_by_function_code(function_code, &bytes);
...
fn test_extract_payload_by_function_code() {
    ...
    let result_data_1: Option<Vec<u8>> = extract_payload_by_function_code(0x03, &test_data_1);
...
fn test_verify_function_code() {
    ...
    let test_data_request: Option<ModbusTelegram> =
        ModbusTelegram::new(0x00A0, 0x01, 0x01, &l_payload, 0x00);
    assert!(test_data_request.is_some());

    let test_data_response: Option<ModbusTelegram> =
        ModbusTelegram::new(0x00A0, 0x01, 0x01, &l_payload, 0x00);
    assert!(test_data_response.is_some());

    let is_equal: bool =
        verify_function_code(&test_data_request.unwrap(), &test_data_response.unwrap());
    assert!(is_equal);
}

pub fn verify_function_code(
    request_telegram: &ModbusTelegram,
    response_telegram: &ModbusTelegram,
) -> bool {
