...
fn test_extract_byte_from_bytearray() {
    ...
    let result_1: Option<u8> = extract_byte_from_bytearray(&test_array, 0);
    assert!(result_1.is_some());
    assert_eq!(result_1.unwrap(), 0x01);

    let result_2: Option<u8> = extract_byte_from_bytearray(&test_array, 2);
    assert!(result_2.is_some());
    assert_eq!(result_2.unwrap(), 0xFF);

    let result_3: Option<u8> = extract_byte_from_bytearray(&test_array, 3);
    assert!(result_3.is_some());
    assert_eq!(result_3.unwrap(), 0x10);

    let result_4: Option<u8> = extract_byte_from_bytearray(&test_array, 4);
...
pub fn extract_byte_from_bytearray(source_array: &Vec<u8>, start_index: u8) -> Option<u8> {
    ...
    if let Some(result) = extract_bytes_from_bytearray(&source_array, start_index, 1) {
        reply = Some(result[0]);
    } else {
        reply = None;
    }
...
fn test_extract_bytes_from_bytearray() {
    ...
    let result_1: Option<Vec<u8>> = extract_bytes_from_bytearray(&test_array, 0, 1);
    assert!(result_1.is_some());
    assert_eq!(result_1.unwrap()[0], 0x01);

    let result_2: Option<Vec<u8>> = extract_bytes_from_bytearray(&test_array, 1, 2);
    assert!(result_2.is_some());
    ...
    let result_3: Option<Vec<u8>> = extract_bytes_from_bytearray(&test_array, 1, 4);
    assert!(result_3.is_none());

    let result_4: Option<Vec<u8>> = extract_bytes_from_bytearray(&test_array, 3, 1);
    assert!(result_4.is_some());
    assert_eq!(result_4.unwrap()[0], 0x10);

    let result_5: Option<Vec<u8>> = extract_bytes_from_bytearray(&test_array, 3, 2);
    assert!(result_5.is_none());

    let result_6: Option<Vec<u8>> = extract_bytes_from_bytearray(&test_array, 4, 1);
...
fn test_extract_word_from_bytearray() {
    ...
    let result_1: Option<u16> = extract_word_from_bytearray(&test_array, 1);
    assert!(result_1.is_some());
    assert_eq!(result_1.unwrap(), 0x0AFF);

    let result_2: Option<u16> = extract_word_from_bytearray(&test_array, 3);
...
pub fn extract_word_from_bytearray(source_array: &Vec<u8>, start_index: u8) -> Option<u16> {
    ...
    if let Some(result) = extract_bytes_from_bytearray(&source_array, start_index, 2) {
        reply = Some(transform_bytes_to_word(&result, 0));
    } else {
        reply = None;
    }
